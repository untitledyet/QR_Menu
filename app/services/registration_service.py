...
def send_verification_email(email: str, token: str, venue_name: str) -> bool:
    ...
    try:
        ...
        msg['Subject'] = "Tably — ელ. ფოსტის დადასტურება"
        ...
        <div style="font-family:Inter,sans-serif;max-width:480px;margin:0 auto;padding:32px;">
            ...
            <p>გამარჯობა! <strong>{venue_name}</strong>-ის რეგისტრაცია თითქმის დასრულდა.</p>
            <p>დაადასტურეთ ელ. ფოსტა:</p>
            <a href="{verify_url}" style="display:inline-block;padding:12px 24px;background:#FF6B35;color:#fff;border-radius:8px;text-decoration:none;font-weight:600;">
                ელ. ფოსტის დადასტურება
            </a>
            <p style="color:#888;font-size:12px;margin-top:24px;">ლინკი მოქმედებს 24 საათი.</p>
        </div>
        """
        ...
        # Still print link in console as fallback
        base_url = os.environ.get('BASE_URL', 'http://localhost:5001')
        verify_url = f"{base_url}/verify-email/{token}"
        print(f"[EMAIL FALLBACK] Verification link: {verify_url}")
        return False
